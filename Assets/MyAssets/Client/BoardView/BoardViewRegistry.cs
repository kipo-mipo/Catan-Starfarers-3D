...
public class BoardViewRegistry : MonoBehaviour
{
    public static BoardViewRegistry Instance { get; private set; }

    // The physics scene that owns the board objects (critical for multi-scene / Mirror setups)
    public PhysicsScene physicsScene;

    // Lookups by ID
    public readonly Dictionary<int, BoardNodeView> Nodes = new Dictionary<int, BoardNodeView>();
    public readonly Dictionary<int, BoardLaneView> Lanes = new Dictionary<int, BoardLaneView>();
    ...
    void Start()
    {
        ...
        Debug.Log($"[BoardViewRegistry] START scene={s.name} loaded={s.isLoaded} physValid={physicsScene.IsValid()} Nodes={Nodes.Count} Lanes={Lanes.Count}");
    }
    ...
    void Awake()
    {
        ...
        {
            Debug.LogWarning("[BoardViewRegistry] Duplicate detected, destroying this instance.");
            Destroy(gameObject);
            return;
        }
        Instance = this;
        ...
        Debug.Log($"[BoardViewRegistry] scene={s.name} sceneValid={s.IsValid()} loaded={s.isLoaded} physValid={physicsScene.IsValid()} Nodes={Nodes.Count} Lanes={Lanes.Count}");
    }
    ...
    public void Rebuild()
    {
        ...
        var nodes = GetComponentsInChildren<BoardNodeView>(true);
        foreach (var n in nodes)
        {
            int id = n.nodeId;
            if (Nodes.ContainsKey(id))
            {
                Debug.LogError($"[BoardViewRegistry] Duplicate BoardNodeView id={id} on '{n.name}' and '{Nodes[id].name}'");
                continue;
            }
            Nodes.Add(id, n);
        }

        var lanes = GetComponentsInChildren<BoardLaneView>(true);
        foreach (var l in lanes)
        {
            int id = GetLaneIdSafe(l);
            if (Lanes.ContainsKey(id))
            {
                Debug.LogError($"[BoardViewRegistry] Duplicate BoardLane id={id} on '{l.name}' and '{Lanes[id].name}'");
                continue;
            }
            Lanes.Add(id, l);
            ...
            {
                Debug.LogWarning($"[BoardViewRegistry] Could not determine endpoints for lane '{l.name}'. AreAdjacent() may fail for it.");
            }
        }
    }
    ...
    public bool TryGetNode(int nodeId, out BoardNodeView node) => Nodes.TryGetValue(nodeId, out node);
    public bool TryGetLane(int laneId, out BoardLaneView lane) => Lanes.TryGetValue(laneId, out lane);
    ...
    private static int GetLaneIdSafe(BoardLaneView lane)
    {
    ...
    private static bool TryGetLaneEndpoints(BoardLaneView lane, out int a, out int b)
    {
        a = 0; b = 0;
        ...
        // 3) Try BoardNodeView references, then pull nodeId from them
    ...
    private static bool TryGetBoardNodeIdRef(object obj, string[] names, out int nodeId)
    {
        ...
        {
            var f = t.GetField(n, BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
            if (f != null && typeof(BoardNodeView).IsAssignableFrom(f.FieldType))
            {
                var bn = f.GetValue(obj) as BoardNodeView;
                if (bn != null) { nodeId = bn.nodeId; return true; }
            }

            var p = t.GetProperty(n, BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
            if (p != null && typeof(BoardNodeView).IsAssignableFrom(p.PropertyType) && p.CanRead)
            {
                var bn = p.GetValue(obj) as BoardNodeView;
                if (bn != null) { nodeId = bn.nodeId; return true; }
            }
        }
